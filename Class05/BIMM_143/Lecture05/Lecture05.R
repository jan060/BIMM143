...
plot(weight$Age, weight$Weight,
     type = "o", #Plot type is "overlapping" (i.e. both dot and line)
     pch = 15, #Point character is filled in squares
     cex = 1.5, #Points are 1.5x bigger than default (i.e. expansion factor)
     lwd = 2, #Line width is 2x bigger than default
     ylim = c(2, 10), 
     xlab = "Age (months)", 
     ylab = "Weight (kg)", 
...
express <- read.delim("bimm143_05_rstats/up_down_expression.txt")
table(express$State)
